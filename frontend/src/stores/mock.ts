import type { Project, Issue, ActionItem, Deliverable, Meeting, OfficialDoc } from '@/types'

export const mockProjects: Project[] = [
  {
    id: 1,
    name: '차세대 스마트홈 IoT 플랫폼 구축',
    description: '가정용 디바이스 연동 및 지능형 가전 원격 제어를 위한 고가용성 클라우드 기반 IoT 플랫폼 설계 및 구축 프로젝트.',
    team: '개발팀',
    stage: 'EXECUTION',
    status: 'IN_PROGRESS',
    statusLabel: '수행 중',
    client: '국립정보자원관리원',
    pm: '안유경',
    pmId: 1,
    startDate: '2026-03-02',
    endDate: '2026-08-31',
    budget: 2450000000,
    progress: 65,
    memberCount: 15,
    deliverableTotal: 3,
    deliverableSubmitted: 2,
    deliverableReview: 1,
    riskLevel: '보통',
    dday: -76,
  },
  {
    id: 2,
    name: 'AI 기반 다국어 고객 상담 어시스턴트 개발',
    description: 'LLM(대형 언어 모델) 파인튜닝 기법을 활용하여 실시간 고객 문의 분석 및 조언 답변 생성 서비스를 구축하는 프로젝트.',
    team: '기획팀',
    stage: 'EXECUTION',
    status: 'DELAYED',
    statusLabel: '지연',
    client: '국민건강보험공단',
    pm: '이영희',
    pmId: 2,
    startDate: '2026-04-10',
    endDate: '2026-05-31',
    budget: 1800000000,
    progress: 45,
    memberCount: 8,
    deliverableTotal: 2,
    deliverableSubmitted: 1,
    deliverableReview: 1,
    riskLevel: '높음',
    dday: 15,
  },
]

export const mockBiddingProjects: Project[] = [
  {
    id: 10,
    name: '전사 통합 ERP 시스템 고도화 및 클라우드 이전',
    description: '레거시 온프레미스 ERP 핵심 인프라를 AWS 클라우드로 이관하고 고도화하는 사업.',
    team: '개발팀',
    stage: 'BIDDING',
    status: 'BIDDING',
    statusLabel: '입찰 제안',
    bidStatus: 'PREPARING',
    client: '조달청(행정안전부)',
    pm: '김철수',
    pmId: 3,
    startDate: '2026-06-15',
    endDate: '2026-06-30',
    budget: 8900000000,
    progress: 0,
    memberCount: 5,
    deliverableTotal: 0,
    deliverableSubmitted: 0,
    deliverableReview: 0,
    riskLevel: '보통',
    dday: -14,
    consortiumRole: '미지정',
    consortiumShare: null,
    vrb: 'VRB : 미상신',
    announcementNo: null,
    proposalDeadline: '2026-06-30',
  },
  {
    id: 11,
    name: '대법원 차세대 동기기정보시스템 구축 및 인프라 보강',
    description: '',
    team: '개발팀',
    stage: 'BIDDING',
    status: 'BIDDING',
    statusLabel: '제안 제출',
    bidStatus: 'SUBMITTED',
    client: '대법원',
    pm: '안유경',
    pmId: 1,
    startDate: '2026-06-15',
    endDate: '2026-06-30',
    budget: 4200000000,
    progress: 0,
    memberCount: 3,
    deliverableTotal: 0,
    deliverableSubmitted: 0,
    deliverableReview: 0,
    riskLevel: '보통',
    dday: -2,
    consortiumRole: '미지정',
    consortiumShare: null,
    vrb: 'VRB : 미상신',
    announcementNo: null,
    proposalDeadline: '2026-06-30',
  },
]

export const mockIssues: Issue[] = [
  {
    id: 1,
    projectId: 1,
    projectName: '차세대 스마트홈 IoT 플랫폼 구축',
    type: 'ISSUE',
    title: 'IoT 기기 통신 지연 및 동시 접속 병목 현상',
    priority: '상',
    assignee: '안유경',
    occurredDate: '2026-05-15',
    status: 'IN_PROGRESS',
    statusLabel: '조치중',
  },
  {
    id: 2,
    projectId: 2,
    projectName: 'AI 기반 다국어 고객 상담 어시스턴트 개발',
    type: 'RISK',
    title: '상담 로그 데이터 개인정보 비식별 조치 누락 위험',
    priority: '중',
    assignee: '이영희',
    occurredDate: '2026-05-20',
    status: 'DONE',
    statusLabel: '완료',
  },
  {
    id: 3,
    projectId: 2,
    projectName: 'AI 기반 다국어 고객 상담 어시스턴트 개발',
    type: 'ISSUE',
    title: 'GPU 서버 납품 일정 지연에 따른 모델 학습 차질',
    priority: '상',
    assignee: '강동우',
    occurredDate: '2026-05-28',
    status: 'OPEN',
    statusLabel: '발생',
  },
]

export const mockActionItems: ActionItem[] = [
  {
    id: 1,
    projectId: 1,
    projectName: '차세대 스마트홈 IoT 플랫폼 구축',
    title: 'IoT Core 서비스 API 규격서 v1.2 확정',
    assignee: '김준현',
    dueDate: '2026-06-05',
    completedDate: null,
    status: 'IN_PROGRESS',
    statusLabel: '진행중',
  },
  {
    id: 2,
    projectId: 2,
    projectName: 'AI 기반 다국어 고객 상담 어시스턴트 개발',
    title: 'LLM 파인튜닝용 상담 로그 데이터 정제',
    assignee: '강동우',
    dueDate: '2026-05-30',
    completedDate: '2026-05-29',
    status: 'DONE',
    statusLabel: '완료',
  },
  {
    id: 3,
    projectId: 1,
    projectName: '차세대 스마트홈 IoT 플랫폼 구축',
    title: '개발환경 보안 진단 점검 조치계획서 제출',
    assignee: '안유경',
    dueDate: '2026-06-08',
    completedDate: null,
    status: 'PENDING',
    statusLabel: '대기',
  },
]

export const mockDeliverables: Deliverable[] = [
  {
    id: 1,
    projectId: 1,
    name: 'IoT 플랫폼 요구사항 정의서',
    category: '요구사항정의서',
    version: 'v1.0.0',
    author: '안유경',
    updatedAt: '2026-03-28',
    status: 'APPROVED',
    statusLabel: '승인 완료',
  },
  {
    id: 2,
    projectId: 1,
    name: '클라우드 인프라 및 DB 아키텍처 설계서',
    category: '시스템설계서',
    version: 'v1.1.0',
    author: '박지민',
    updatedAt: '2026-04-19',
    status: 'APPROVED',
    statusLabel: '승인 완료',
  },
  {
    id: 3,
    projectId: 1,
    name: 'IoT API 서버 게이트웨이 모듈 소스코드',
    category: '소스코드',
    version: 'v0.9.0',
    author: '김준현',
    updatedAt: '2026-05-24',
    status: 'REVIEW',
    statusLabel: '검토 요청',
  },
]

export const mockMeetings: Meeting[] = [
  {
    id: 1,
    projectId: 1,
    projectName: '차세대 스마트홈 IoT 플랫폼 구축',
    title: '차세대 스마트홈 IoT 플랫폼 요구사항 확정 회의',
    type: '의사결정 회의',
    dateTime: '2026-03-10 14:00',
    location: '오케스트로 본사 5층 대회의실',
    attendees: '오케스트로 안유경 PM, 박지민 대리, 고객사 김철수 부장 외 2명',
    status: 'COMPLETED',
  },
  {
    id: 2,
    projectId: 2,
    projectName: 'AI 기반 다국어 고객 상담 어시스턴트 개발',
    title: 'LLM 서비스 모델 아키텍처 및 GPU 서버 계획 회의',
    type: '정기 회의',
    dateTime: '2026-04-20 10:30',
    location: '고객사 스마트홈 회의실',
    attendees: '오케스트로 이영희 PM, 강동우 연구원, 국민건강보험공단 박상무 외 2명',
    status: 'COMPLETED',
  },
]

export const mockOfficialDocs: OfficialDoc[] = [
  {
    id: 1,
    projectId: 1,
    projectName: '차세대 스마트홈 IoT 플랫폼 구축',
    docNo: 'OKE-202603-000312',
    title: '차세대 스마트홈 IoT 플랫폼 구축 착수계 제출의 건',
    direction: 'INBOUND',
    senderOrg: '오케스트로_전출',
    receiverOrg: '국가정보자원관리원 서울센터',
    drafter: '안유경',
    draftDept: '오케스트로_전출',
    sentDate: '2026-03-05',
    approvalStatus: 'APPROVED',
    reviewStatus: 'APPROVED',
    attachmentCount: 2,
  },
  {
    id: 2,
    projectId: 1,
    projectName: '차세대 스마트홈 IoT 플랫폼 구축',
    docNo: 'OKE-202605-002165',
    title: '「2026년 대구센터 클라우드 자원물 운영·유지관리 사업」가상화 소프트웨어 보안취약점 14개 항목 예외 조치 요청의 건',
    direction: 'INBOUND',
    senderOrg: '오케스트로_전출',
    receiverOrg: '국가정보자원관리원 대구센터',
    drafter: '안유경',
    draftDept: '오케스트로_전출',
    sentDate: '2026-05-27',
    approvalStatus: 'APPROVED',
    reviewStatus: 'APPROVED',
    attachmentCount: 4,
  },
  {
    id: 3,
    projectId: 2,
    projectName: 'AI 기반 다국어 고객 상담 어시스턴트 개발',
    docNo: 'OKE-202605-002210',
    title: 'AI 기반 다국어 고객 상담 시스템 중간보고서 제출의 건',
    direction: 'OUTBOUND',
    senderOrg: '오케스트로AI솔루션_팀',
    receiverOrg: '국민건강보험공단 IT기획부',
    drafter: '이영희',
    draftDept: '오케스트로AI솔루션_팀',
    sentDate: null,
    approvalStatus: 'PENDING',
    reviewStatus: 'PENDING',
    attachmentCount: 1,
  },
]
